...
| `displayHoursBeforeRain`        | If set to a value greater than zero, the map is only displayed when rain is expected within the defined number of hours. If set to zero, the map is only shown if the current weather shows rain. If set to -1, the map is always displayed.<br>Supported weather modules are: [weather](https://github.com/MichMich/MagicMirror/tree/master/modules/default/weather) (with type `current` if set to zero and type `hourly` for other values) and [MMM-OpenWeatherForecast](https://github.com/jclarke0000/MMM-OpenWeatherForecast) (only if set to zero). <br><br>**Type:** `number` <br> **Default value:** `-1` |
